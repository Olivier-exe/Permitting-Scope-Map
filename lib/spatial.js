...
export function findNearby(items, lat, lng, threshold) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    if (minDist < threshold) {
      ...
      if (bestSeg) { var cp = closestOnSegment(lat, lng, bestSeg); entry.brg = compass8(bearing(lat, lng, cp[0], cp[1])); }
      out.push(entry);
    }
  }
...
export function runFullAnalysis(lat, lng, layerData, company, manifest) {
  ...
  if (layerData[dotKey]) result.dot = findNearby(layerData[dotKey], lat, lng, 500).slice(0, 8).map(function(h) { return { n: h.n || 'Road', t: h.t === 'I' ? 'Interstate' : h.t === 'U' ? 'US Highway' : 'State/Local', d: h.dist, mp: h.p || '', district: h.s || '', dir: h.brg || '' }; });
  var rrKey = company + '_rr';
  if (layerData[rrKey]) result.rr = findNearby(layerData[rrKey], lat, lng, 800).slice(0, 5).map(function(r) { return { o: r.o || r.n || 'Railroad', d: r.dist, dir: r.brg || '' }; });
