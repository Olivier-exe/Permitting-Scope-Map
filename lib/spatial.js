...
export function compass8(b) {
  var dirs = ['N','NE','E','SE','S','SW','W','NW'];
  return dirs[Math.round(((b % 360) + 360) % 360 / 45) % 8];
}

function closestOnSegment(lat, lng, s) {
  var ml = (s[0] + s[2]) / 2, cl = Math.cos(ml * Math.PI / 180);
  var x = (lng - s[1]) * cl, y = lat - s[0];
  var x2 = (s[3] - s[1]) * cl, y2 = s[2] - s[0];
  var l = x2 * x2 + y2 * y2;
  var t = l === 0 ? 0 : Math.max(0, Math.min(1, (x * x2 + y * y2) / l));
...
// Same contract as findNearby but with a cached bounding-box prefilter so it
// stays fast on very large layers (national transmission: ~94K lines).
export function findNearbyFast(items, lat, lng, threshold) {
  if (!items._bb) {
    var bb = new Array(items.length);
    for (var b = 0; b < items.length; b++) {
      var p = items[b].c; if (!p || p.length < 2) { bb[b] = null; continue; }
      var la0 = 90, la1 = -90, lo0 = 180, lo1 = -180;
      for (var q = 0; q < p.length; q++) {
        if (p[q][0] < la0) la0 = p[q][0]; if (p[q][0] > la1) la1 = p[q][0];
        if (p[q][1] < lo0) lo0 = p[q][1]; if (p[q][1] > lo1) lo1 = p[q][1];
      }
      bb[b] = [la0, la1, lo0, lo1];
    }
    try { Object.defineProperty(items, '_bb', { value: bb, enumerable: false }); } catch (e) { items._bb = bb; }
  }
  var mLat = threshold / 111320;
  var mLng = threshold / (111320 * Math.max(0.2, Math.cos(lat * Math.PI / 180)));
  var cand = [];
  for (var i = 0; i < items.length; i++) {
    var box = items._bb[i]; if (!box) continue;
    if (lat < box[0] - mLat || lat > box[1] + mLat || lng < box[2] - mLng || lng > box[3] + mLng) continue;
    cand.push(items[i]);
  }
  return findNearby(cand, lat, lng, threshold);
}

export function findNearby(items, lat, lng, threshold) {
  var out = [];
  for (var i = 0; i < items.length; i++) {
    var item = items[i], minDist = Infinity, pts = item.c, bestSeg = null;
    if (!pts || pts.length < 2) continue;
    for (var j = 0; j < pts.length - 1; j++) {
      var seg = [pts[j][0], pts[j][1], pts[j + 1][0], pts[j + 1][1]];
      var d = distToSegment(lat, lng, seg);
    ...
    if (minDist < threshold) {
      ...
      if (item.w) entry.w = item.w;
      entry.dist = Math.round(minDist);
      if (bestSeg) { var cp = closestOnSegment(lat, lng, bestSeg); entry.brg = compass8(bearing(lat, lng, cp[0], cp[1])); entry.cp = [Math.round(cp[0]*1e6)/1e6, Math.round(cp[1]*1e6)/1e6]; }
      out.push(entry);
    }
  }
...
export function findNearbyPoints(items, lat, lng, threshold) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    if (d < threshold) out.push({ n: item.n || 'Airport', dist: Math.round(d), brg: compass8(bearing(lat, lng, item.lat, item.lng)) });
  }
  return out.sort(function(a, b) { return a.dist - b.dist; });
}
...
export function runFullAnalysis(lat, lng, layerData, company, manifest) {
  ...
  var result = { lat: lat, lng: lng, city: null, county: null, state: null, dot: [], rr: [], rrx: [], transmission: [], usTx: [], levee: [], faa: [], parish_roads: [], row: null };
  ...
  var rxKey = company + '_rr_crossings';
  if (layerData[rxKey]) result.rrx = findNearbyPoints(layerData[rxKey], lat, lng, 1600).slice(0, 3);
  if (layerData[dotKey]) result.dot = findNearby(layerData[dotKey], lat, lng, 500).slice(0, 8).map(function(h) { return { n: h.n || 'Road', t: h.t === 'I' ? 'Interstate' : h.t === 'U' ? 'US Highway' : 'State/Local', d: h.dist, mp: h.p || '', district: h.s || '', dir: h.brg || '', cp: h.cp || null, w: h.w || 0 }; });
  var rrKey = company + '_rr';
  if (layerData[rrKey]) result.rr = findNearby(layerData[rrKey], lat, lng, 800).slice(0, 5).map(function(r) { return { o: r.o || r.n || 'Railroad', d: r.dist, dir: r.brg || '', cp: r.cp || null }; });
  var txKey = company + '_transmission';
  if (layerData[txKey]) result.transmission = findNearby(layerData[txKey], lat, lng, 800).slice(0, 5).map(function(t) { return { n: t.n || 'Transmission', v: t.v || '', d: t.dist, dir: t.brg || '' }; });
  // National grid supplement (HIFLD/EIA): display data joins the analysis, but
  // curated circuits stay primary; near-identical hits are deduped by distance.
  var natHits = [];
  ['us_tx_345', 'us_tx_230', 'us_tx_100', 'us_tx_low'].forEach(function(bk) {
    var bd = layerData[company + '_' + bk];
    if (bd) natHits = natHits.concat(findNearbyFast(bd, lat, lng, 800));
  });
  if (natHits.length) {
    natHits.sort(function(a, b) { return a.dist - b.dist; });
    var dedup = [];
    for (var nh = 0; nh < natHits.length && dedup.length < 5; nh++) {
      var h = natHits[nh], dup = false;
      for (var ct = 0; ct < result.transmission.length; ct++) if (Math.abs(result.transmission[ct].d - h.dist) < 60) { dup = true; break; }
      for (var dj = 0; dj < dedup.length; dj++) if (dedup[dj].n === (h.n || 'Transmission') && Math.abs(dedup[dj].d - h.dist) < 25) { dup = true; break; }
      if (!dup) dedup.push({ n: h.n || 'Transmission', d: h.dist, dir: h.brg || '' });
    }
    result.usTx = dedup;
  }
  var lvKey = company + '_levee';
  if (layerData[lvKey]) result.levee = findNearby(layerData[lvKey], lat, lng, 800).slice(0, 5).map(function(l) { return { n: l.n || 'Levee', s: l.s || '', d: l.dist, dir: l.brg || '' }; });
...
export function generatePermits(r) {
  ...
  if (r.dot.length) {
    ...
    pm.push({ type: 'DOT Utility Permit', jurisdiction: nearest.district ? 'FDOT District ' + nearest.district : 'State DOT', recommendation: nearest.n + ' (' + nearest.t + ') is ' + nearest.d + 'm away' + (nearest.dir ? ' to the ' + nearest.dir : '') + (nearest.mp ? ' near MP ' + nearest.mp : '') + (nearest.w ? ' \u00b7 ~' + Math.max(0, Math.round(nearest.d*3.28084 - nearest.w/2)) + 'ft to EOP (est.)' : ''), priority: prio, notes: nearest.d < 30 ? 'Within ROW - permit required' : null });
  }
  if (r.rr.length) {
    var rr = r.rr[0];
    pm.push({ type: 'Railroad Crossing Permit', jurisdiction: rr.o, recommendation: rr.o + ' track ' + rr.d + 'm away' + (rr.dir ? ' to the ' + rr.dir : '') + (r.rrx && r.rrx.length ? ' \u00b7 nearest crossing #' + r.rrx[0].n.replace('Xing ','') + ' (' + Math.round(r.rrx[0].dist*3.28084) + 'ft ' + (r.rrx[0].brg||'') + ')' : ''), priority: rr.d < 50 ? 'Critical' : 'High', notes: 'Contact railroad company + LPSC notification' });
  }
  ...
  } else if (r.usTx && r.usTx.length) {
    var utx = r.usTx[0];
    pm.push({ type: 'Transmission Easement', jurisdiction: utx.n || 'Utility', recommendation: 'Transmission line ' + utx.d + 'm away (US grid data)', priority: utx.d < 50 ? 'Critical' : 'High', notes: 'From national dataset \u2014 verify line owner before contact' });
  }
  if (r.levee.length) {
    var lv = r.levee[0];
