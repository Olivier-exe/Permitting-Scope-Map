...
export function runFullAnalysis(lat, lng, layerData, company, manifest) {
  ...
  if (layerData[dotKey]) result.dot = findNearby(layerData[dotKey], lat, lng, 500).slice(0, 8).map(function(h) { return { n: h.n || 'Road', t: h.t === 'I' ? 'Interstate' : h.t === 'U' ? 'US Highway' : 'State/Local', d: h.dist, mp: h.p || '', district: h.s || '' }; });
...
export function generatePermits(r) {
  ...
  if (r.dot.length) {
    ...
    pm.push({ type: 'DOT Utility Permit', jurisdiction: nearest.district ? 'FDOT District ' + nearest.district : 'State DOT', recommendation: nearest.n + ' (' + nearest.t + ') is ' + nearest.d + 'm away' + (nearest.mp ? ' near MP ' + nearest.mp : ''), priority: prio, notes: nearest.d < 30 ? 'Within ROW - permit required' : null });
  }
  if (r.rr.length) {
    var rr = r.rr[0];
