              ...
              <div style={{padding:'12px 14px'}}>
                {tab==='info'?<>
                  <div style={{background:'rgba(255,255,255,0.04)',border:'1px solid var(--glass-border)',borderRadius:10,padding:12,marginBottom:10}}>
                    ...
                    {[['Coords',selPt.lat.toFixed(5)+', '+selPt.lng.toFixed(5),'var(--text)'],['State',sel.info.state||'\u2014','var(--text)'],[sel.info.state==='Louisiana'?'Parish':'County',sel.info.county||'\u2014','#4a9eff'],['City',sel.info.city||'Unincorporated','var(--success)']].concat(sel.info.row?[['ROW',sel.info.row,'#f472b6']]:[]).concat(sel.info.elevation?[['Elev',sel.info.elevation.ft.toFixed(1)+' ft','#06b6d4']]:[])
                  ...
                  <DetailBox title={sel.info.faa.length?'Airports ('+sel.info.faa.length+')':'Airports \u2014 Clear'} color="#06b6d4">{sel.info.faa.map(function(a,i){return <NearbyRow key={i} label={a.n} dist={a.dist}/>;})}{!sel.info.faa.length&&<div style={{fontSize:11,color:'var(--muted)'}}>None within 10,000ft</div>}</DetailBox>
                </>:<>
